...
document.addEventListener('motd.form.init', (e) => {
    ...
    form.addEventListener('change', e => {
        if (e.target.matches('[name="repeat_end_type"]') || e.target.matches('[name="repeat"]')) {
            updateViewSwitchers();
        }
    });
    ...
    function updateViewSwitchers() {
        ...
        if (switcher.repeat_end_type) {
            switcher.repeat_end_type.mainObj.value = switcher.repeat_end_type.mainObj.querySelector(':checked').value;
            switcher.repeat_end_type.rebuildView.apply(switcher.repeat_end_type);
        }
